...
class MemberCell: ParentSectionCell {
    ...
    override func awakeFromNib() {
        ...
        lblType.setRounded(cornerRadius: 10, width: 2, color: AppTheme.lightBlue())
    }
    ...
    override func populateData(viewModel: AnyObject, _ index: Int = 0) {
        ...
        if let vm = viewModel as? CategoryCreationVM {
            ...
            if vm.categoryUsers.value.count > 0 {
                username = vm.categoryData.value.owner?.name ?? ""
                userId   = vm.categoryData.value.owner?.id ?? ""
                
                if index > 0 {
                    username = vm.categoryUsers.value[index-1].name ?? vm.categoryUsers.value[index-1].email ?? ""
                    userId = vm.categoryUsers.value[index-1].id ?? ""
                }
