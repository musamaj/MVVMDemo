...
extension TaskDetailVM {
    ...
    func fetchSubtasks() {
        ...
        self.subtaskItems   = SubtaskEntity.getSubTasks(true, sortAscending: false, byTask: true)
    ...
    func deleteSubtask(subtaskId: String, index: Int) {
        ...
        //SubtaskEntity.deleteSubTask(item ?? SubtaskEntity())
        if let object = item {
...
extension TaskDetailVM {
    ...
    func handleSubtaskUpdate() {
        SubtaskEntity.fetchId = self.subtaskData.value.id
        SubtaskEntity.updateSubtask(subTaskLD.subtaskData, true)
    }
    
    func handleSubtaskCreate() {
        SubtaskEntity.fetchId = self.subtaskData.value.id
        SubtaskEntity.updateSubtask(subTaskLD.subtaskData, true)
        self.fetchSubtasks()
    }
    
    func handleSubtaskDelete() {
        // nothing here
    }
